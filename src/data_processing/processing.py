...
def extract_parentheses(title):
        ...
        return match.group(1) if match else title  # Retourne le texte entre parenthèses s'il existe, sinon le titre original
...
def processing_data (df_attraction,df_michelin,country,city, threshold=80):
    ...
    df_attraction_clean[['latitude', 'longitude']] = df_attraction_clean['Title'].apply(lambda x: geocode_addresses(x,country,city))
    df_attraction_clean.drop(df_attraction_clean[df_attraction_clean['latitude'].isna() | df_attraction_clean['longitude'].isna()].index, inplace=True)
