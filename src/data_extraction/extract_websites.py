...
def BucketList_attractions(soup):
    ...
    for article in articles:
        ...
        df_BucketList = pd.concat(
            [df_BucketList, pd.DataFrame([attraction_info])], ignore_index=True
        )
...
def WorldTravelGuide_attractions(soup):
    ...
    for article in articles:
        ...
        if disabled_access_tag:
            ...
            disabled_access_text = (
                disabled_access_tag.next_sibling.strip()
                if disabled_access_tag.next_sibling
                else "Non spécifié"
            )
            attraction_info["Disabled Access"] = disabled_access_text
        df_WorldTravelGuide = pd.concat(
            [df_WorldTravelGuide, pd.DataFrame([attraction_info])], ignore_index=True
        )
...
def CNTraveler_attractions(soup):
    ...
    for article in articles:
        ...
        title_tag = article.find(
            "span", class_="GallerySlideCaptionHedText-iqjOmM jwPuvZ"
        )
        title = title_tag.get_text(strip=True) if title_tag else "Titre non trouvé"
        attraction_info = {"Title": title}
        description_tag = article.find("p")
        if description_tag:
            attraction_info["description"] = description_tag.get_text(strip=True)
        df_CNTraveler = pd.concat(
            [df_CNTraveler, pd.DataFrame([attraction_info])], ignore_index=True
        )
...
def Routard_attractions(soup):
    ...
    articles = soup.find_all(
        "div", class_="bg-rtd-grey-100 flex h-96 w-60 flex-col rounded-xl p-4"
    )
    for article in articles:
        # Extraire le titre de l'attraction
        title_tag = article.find(
            "h2", class_="group-hover:text-rtd-green my-2 font-semibold"
        )
        title = title_tag.get_text(strip=True) if title_tag else "Titre non trouvé"
        attraction_info = {"Title": title}
        ...
        if description_tag:
            attraction_info["description"] = description_tag.get_text(strip=True)
        df_Routard = pd.concat(
            [df_Routard, pd.DataFrame([attraction_info])], ignore_index=True
        )
...
def translate_location(name, src_lang="en", dest_lang="fr"):
    ...
    try:
        ...
        text = "".join(
            char for char in translation if unicodedata.category(char) != "Mn"
        )
        return text
...
def routard_city_to_region(city):
    ...
    if city in cities_to_regions.keys():
...
def routard_continent(continent):
    ...
    if continent in routard_continent_fr.keys():
...
def extract_websites(
    ...
    if country == "france":
        ...
        URL_dict["Routard"] = (
            f"https://www.routard.com/fr/guide/top/{country}/{region}/{city_fr}"
        )
    ...
    for website in websites_to_call:
        ...
        if soup:
            df = pd.concat(
                [df, URL_extractor[f"{website}_attractions"](soup)], ignore_index=True
            )
